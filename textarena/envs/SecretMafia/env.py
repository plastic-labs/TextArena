...
class SecretMafiaEnv(ta.Env):
    ...
    def _day_private_reflection(self, current_pid, action):
        ...
        if content is None:
            ...
            self.state.add_observation(from_id=current_pid, to_id=ta.DEBUG_ID, message=f"[DEBUG] Invalid reflection action: {action}")
            return
    ...
    def _day_discussion(self, current_pid, action):
        ...
        if content is None:
            ...
            self.state.add_observation(from_id=current_pid, to_id=ta.DEBUG_ID, message=f"[DEBUG] Invalid discussion action: {action}")
            return
    ...
    def _day_voting(self, current_pid, action):
        ...
        if content is None:
            ...
            self.state.add_observation(from_id=current_pid, to_id=ta.DEBUG_ID, message=f"[DEBUG] Invalid vote action: {action}")
            return
        ...
        if not match:
            ...
            self.state.add_observation(from_id=current_pid, to_id=ta.DEBUG_ID, message=f"[DEBUG] Invalid vote format: {action}")
            return
    ...
    def _night_mafia(self, current_pid, action):
        ...
        if content is None:
            ...
            self.state.add_observation(from_id=current_pid, to_id=ta.DEBUG_ID, message=f"[DEBUG] Invalid mafia vote action: {action}")
            return
        ...
        if not match:
            self.state.set_invalid_move(player_id=current_pid, reason=f"The vote was not submitted in the correct format.")
            self.state.add_observation(from_id=current_pid, to_id=ta.DEBUG_ID, message=f"[DEBUG] Invalid mafia vote format: {action}")
            return
    ...
    def _night_doctor(self, current_pid, action):
        ...
        if content is None:
            ...
            self.state.add_observation(from_id=current_pid, to_id=ta.DEBUG_ID, message=f"[DEBUG] Invalid protect action: {action}")
            return
        ...
        if not match:
            ...
            self.state.add_observation(from_id=current_pid, to_id=ta.DEBUG_ID, message=f"[DEBUG] Invalid protect format: {action}")
            return
    ...
    def _night_detective(self, current_pid, action):
        ...
        if content is None:
            ...
            self.state.add_observation(from_id=current_pid, to_id=ta.DEBUG_ID, message=f"[DEBUG] Invalid investigate action: {action}")
            return
            
        match = self.voting_pattern.search(content)
        if not match:
            self.state.set_invalid_move(player_id=current_pid, reason=f"The action was not submitted in the correct format.")
            self.state.add_observation(from_id=current_pid, to_id=ta.DEBUG_ID, message=f"[DEBUG] Invalid investigate format: {action}")
            return
    ...
    def _night_mafia_discussion(self, current_pid, action):
        ...
        if content is None:
            ...
            self.state.add_observation(from_id=current_pid, to_id=ta.DEBUG_ID, message=f"[DEBUG] Invalid mafia suggest action: {action}")
            return
        ...
        if not match:
            ...
            self.state.add_observation(from_id=current_pid, to_id=ta.DEBUG_ID, message=f"[DEBUG] Invalid mafia suggest format: {action}")
            return
